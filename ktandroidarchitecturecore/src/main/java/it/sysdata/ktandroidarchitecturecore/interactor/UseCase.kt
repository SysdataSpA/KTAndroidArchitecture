...
import it.sysdata.ktandroidarchitecturecore.BaseConfig
 ...
 * (kotlin coroutine) and will post the result in the CommonPool or UI thread.
 */
abstract class UseCase<out Type, in Params> where Type : Any, Params : ActionParams {
    ...
    fun execute(
        ...
        scope.launch(Dispatchers.Default, CoroutineStart.DEFAULT) {
            ...
            if (safeExecute)
                onResult.invoke(BaseConfig.safeExecutor.safeExecute(job))
            else
